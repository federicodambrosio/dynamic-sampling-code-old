	...
	for (double logN = minlogN; logN < maxlogN; logN += deltalogN) {
		...
		if (large) {
			for (double logratio = minlogratio; logratio < maxlogratio; logratio += deltalogratio) {
				...
				if (doSSACR) for (int i = 0; i < N; i++) SSACRLargeItems.push_back(ssacr.addElement(i, rates.at(i)));
				if (doSSACR) ssacr.update();
				...
				for (int t = 0; t < maxRepetitions; t++) {
					...
					if (doSSAR) {
						...
						countUpdate = 0;
						ssar.updateElement(iit, randomRate, countUpdate);
					...
					if (doSSACR) {
						...
						m = ssacr.sampleElement();
						countUpdate = 0;
						ssacr.updateElement(m, randomRate, countUpdate);
						ssacr.update(countUpdate);
				...
				for (int t = 0; t < maxRepetitions; t++) {
					...
					if (doSSAR) {
						countUpdate = 0;
						ssar.updateElement(SSARLargeItems[randIndex], randomRate, countUpdate);
						arbupdateSSARLarge.Push(countUpdate);
					}
					if (doSSACR) {
						countUpdate = 0;
						ssacr.updateElement(SSACRLargeItems[randIndex], randomRate, countUpdate);
						ssacr.update(countUpdate);
